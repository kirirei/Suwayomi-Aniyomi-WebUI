...
import type { TAnimeTrackRecordBind } from '@/features/tracker/AnimeTracker.types.ts';
import { AvatarSpinner } from '@/base/components/AvatarSpinner.tsx';

/**
 * Mirrors TrackerActiveCard.tsx: episode/total-episode fields replace chapter/total-chapters, and
 * the status list is a fixed MyAnimeList anime vocabulary (WATCHING/PLAN_TO_WATCH/REWATCHING
 * aren't exposed by the manga-typed `tracker.statuses` dataloader field) rather than fetched.
 */
...
const TrackerActiveRemoveBind = ({
    ...
    trackerRecordId: TAnimeTrackRecordBind['id'];
    ...
    const removeBind = () => {
        onClose();
        requestManager
            .unbindAnimeTrack(trackerRecordId, removeRemoteTracking)
            .response.then(() => makeToast(t`Untracked anime`, 'success'))
            .catch((e) => makeToast(t`Could not untrack anime`, 'error', getErrorMessage(e)));
    };

    return (
        <PopupState variant="dialog" popupId={`tracker-active-anime-menu-remove-button-${tracker.id}`}>
            {(popupState) => (
                <>
                    <MenuItem
...
const TrackerUpdatePrivateStatus = ({
    ...
    trackRecordId: TAnimeTrackRecordBind['id'];
    isPrivate: boolean;
    closeMenu: () => void;
    ...
    return (
        <MenuItem
            onClick={() => {
                requestManager
                    .updateAnimeTrackBind(trackRecordId, { private: !isPrivate })
...
type TTrackRecordActive = Pick<TAnimeTrackRecordBind, 'id' | 'remoteUrl' | 'title' | 'private'>;
const TrackerActiveHeader = ({
    ...
    return (
        ...
        >
            ...
            >
                <PopupState variant="popover" popupId={`tracker-active-anime-menu-popup-${tracker.id}`}>
                    {(popupState) => (
                        <>
                            <IconButton {...bindTrigger(popupState)}>
                                <MoreVertIcon />
                            </IconButton>
                            <Menu {...bindMenu(popupState)} id={`tracker-active-anime-menu-${tracker.id}`}>
                                {(onClose, setHideMenu) => [
                                    <TrackerActiveLink
                                        key={`tracker-active-anime-menu-item-browser-${tracker.id}`}
                                        url={trackRecord.remoteUrl}
                                    >
                                        <MenuItem onClick={() => onClose()}>{t`Open in browser`}</MenuItem>
                                    </TrackerActiveLink>,
                                    <TrackerActiveRemoveBind
                                        key={`tracker-active-anime-menu-item-remove-${tracker.id}`}
                                    ...
                                    <TrackerUpdatePrivateStatus
                                        key={`tracker-active-anime-menu-item-private-${tracker.id}`}
...
export const TrackerActiveAnimeCard = ({
    trackRecord,
    tracker,
    onClick,
}: {
    trackRecord: TAnimeTrackRecordBind;
    ...
    const animeStatuses = useMemo(
        () => [
            { value: 11, name: t`Watching` },
            { value: 2, name: t`Completed` },
            { value: 3, name: t`On hold` },
            { value: 4, name: t`Dropped` },
            { value: 16, name: t`Plan to watch` },
            { value: 17, name: t`Rewatching` },
        ],
        [t],
    );

    const displayScore = `${Math.trunc(trackRecord.score)}`;
    const isScoreUnset = isUnsetScore(displayScore);
    const currentScore = isScoreUnset ? tracker.scores[0] : displayScore;
    ...
    const updateTrackerBind = (patch: Parameters<typeof requestManager.updateAnimeTrackBind>[1]) => {
        requestManager
            .updateAnimeTrackBind(trackRecord.id, patch)
            .response.catch((e) => makeToast(t`Failed to save changes`, 'error', getErrorMessage(e)));
    };
                        ...
                        <Box sx={{ padding: 1 }}>
                            <TrackerActiveCardInfoRow>
                                <ListPreference
                                    ...
                                    entries={animeStatuses.map((status) => status.name)}
                                    key="status"
                                    type="ListPreference"
                                    entryValues={animeStatuses.map((status) => `${status.value}`)}
                                ...
                                <NumberSetting
                                    settingTitle={t`Episode`}
                                    dialogTitle={t`Episode`}
                                    settingValue={`${trackRecord.lastEpisodeSeen}/${trackRecord.totalEpisodes}`}
                                    value={trackRecord.lastEpisodeSeen}
                                    minValue={0}
                                    maxValue={Number.MAX_SAFE_INTEGER}
                                    valueUnit=""
                                    handleUpdate={(lastEpisodeSeen) => updateTrackerBind({ lastEpisodeSeen })}
                                />
                                <Divider orientation="vertical" flexItem />
